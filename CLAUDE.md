...
- **Schema:** `src/db/schema.js` — six tables: `users`, `refresh_tokens`, `houses`, `chore_types`, `household_members`, `chore_assignments`. All chore data is scoped by `house_id`. `household_members.user_id` links to `users` (nullable for legacy/guest members).
- **Routes:** `src/routes/` — `auth.js` mounted at `/auth`; `houses.js`, `choreTypes.js`, `members.js`, `assignments.js` all mounted under `/houses/:houseId/...`.
- **Middleware:** `src/middleware/requireAuth.js` validates JWT from `Authorization: Bearer <token>` and attaches `req.user`. `requireHouseMember.js` checks DB membership and attaches `req.member`.
