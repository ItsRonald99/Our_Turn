...
- **Routes:** `src/routes/` — `auth.js` at `/auth` (`POST /register`, `POST /login`, `POST /logout`, `POST /refresh`, `GET /me`, `POST /change-password`, `POST /change-username` — last two require `requireAuth` and verify current password via bcrypt before updating); `houses.js`, `choreTypes.js` (`GET`, `POST {title, description}`, `DELETE /:id` — owner only), `members.js`, `assignments.js` under `/houses/:houseId/...`; `houseInvitations.js` at `/houses/:houseId/invitations`; `invitations.js` at `/invitations`; `notifications.js` at `/notifications` (`GET /notifications`, `POST /notifications/:id/read`); `dashboard.js` at `/houses/:houseId/dashboard` (`GET` — requireAuth + requireHouseMember).
